...
export const modifierBouteilleDuCellier = async (req, res) => {
	try {
		...
		const bouteilleExistante =
			await modeleBouteilleCellier.verifierExistence(
				identifiantCellier,
		...
		if (bouteilleExistante) {
			...
			res.status(404).json({
				message: "Bouteille non trouvée dans le cellier",
			});
		}
	} catch (error) {
		console.error(error);
...
export const ajouterBouteilleDuCellier = async (req, res) => {
	try {
		...
		res.status(201).json({
			message: "Bouteille ajoutée au cellier",
			id: action,
		});
	} catch (error) {
		console.error(error);
...
export const supprimerBouteilleDuCellier = async (req, res) => {
	try {
		...
		if (resultat) {
			res.status(200).json({
				message: "Bouteille supprimée du cellier avec succès",
			});
		} else {
			res.status(404).json({
				message: "Bouteille non trouvée dans le cellier",
