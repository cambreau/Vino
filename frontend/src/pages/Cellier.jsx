...
function Cellier() {
	...
	const mettreAJourQuantite = async (idBouteille, variation) => {
		...
		const bouteilleCible = bouteillesCellier.find(
			(b) => b.id === idBouteille,
		);
	...
	const handleAugmenter = (idBouteille) =>
		mettreAJourQuantite(idBouteille, 1);
	const handleDiminuer = (idBouteille) =>
		mettreAJourQuantite(idBouteille, -1);
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
								{bouteillesCellier.map((bouteille) => (
									<CarteBouteille
										...
										disabled={bouteillesEnTraitement.has(
											bouteille.id,
										)}
									/>
								))}
