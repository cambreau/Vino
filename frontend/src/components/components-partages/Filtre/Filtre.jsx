...
const sontOptionsEgales = (a = DEFAULT_OPTIONS, b = DEFAULT_OPTIONS) => {
  return (
    ...
    JSON.stringify(a.regionsParPays ?? {}) ===
      JSON.stringify(b.regionsParPays ?? {})
  );
};
...
function Filtres({
  ...
  const aDesFiltresActifs = useMemo(
    () =>
      Object.values(criteres).some((v) => Boolean(v)) ||
      Boolean(texteRecherche),
      ...
      >
        <header className="flex items-center justify-between text-(length:--taille-petit) font-semibold text-principal-200 uppercase">
          <span className="flex items-center gap-2">
            <IconRecherche />
            <span>Rechercher une bouteille</span>
          </span>
          <button
            type="button"
              ...
              <div className="relative">
                ...
                >
                  ...
                  {(champ.options ?? []).map((option) => (
                    <option
                      key={`${champ.id}-${formatString(option)}`}
                      value={option}
                    >
                      {option}
                    </option>
